...
description: |
  ...
  En la primera lección, ofrecemos una visión general de este módulo del curso. Cubrimos los principales objetivos y expectativas, presentamos las distintas herramientas que utilizaremos y exponemos algunas de las motivaciones y fundamentos del módulo.
date: "2024-01-11"
...
> If you have not already done so, please read the short post on [Using this Site](../using-this-site/index.html) before continuing.

We will use [Quarto](https://quarto.org/)--an open-source scientific and technical publishing system to build our web products. This site itself is not only the guidebook for your web products but is also ***created with the same tools you will use for your projects***. More on that below.  

In this module of the course, we will learn how to:

1. Set up a working environment by installing [R](https://www.r-project.org/), [RStudio](https://www.rstudio.com/), [Quarto](https://quarto.org/), and various R packages (e.g., [knitr](https://cran.r-project.org/web/packages/knitr/index.html), [tidyverse](https://www.tidyverse.org/), [reactable](https://glin.github.io/reactable/)).
...
5. (if there is interest) Use [git](https://git-scm.com/) and [GitHub Pages](https://pages.github.com/) for hosting our work (**this is not a requirement of the course**).
...
> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"}  More than 70% of researchers have tried and failed to reproduce another scientist's experiments, and more than half have failed to reproduce their own experiments. Those are some of the telling figures that emerged from Nature's survey of 1,576 researchers who took a brief online questionnaire on reproducibility in research. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"} [@baker20161]

::: aside
As of today, this article has been cited at least `r cr_citation_count(doi = "10.1038/533452a")$count` times. [{{< fa lightbulb >}}]{style="color: steelblue;"} ^[To get the number of citations for this article I used the function `cr_citation_count` from the [`rcrossref`](https://cran.r-project.org/web/packages/rcrossref/index.html) package. The function takes a Digital Object Identifier (DOI) as the input and searches CrossRef OpenURL to return the number of citations.]
:::
...
> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"}` … [is a] set of procedures that permit the reader of a paper to see the entire processing trail from the raw data and code to figures and tables. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"} [@goodman2016does]. 

Or the U.S. National Science Foundation (NSF) defines it this way …

> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"}`  … refers to the ability of a researcher to duplicate the results of a prior study using the same materials as were used by the original investigator. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"} [@cacioppo2015social]
...
> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"} … authors are required to make materials, data, code, and associated protocols promptly available to readers without undue qualifications. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"}
...
#| warning: false
#| fig-align: left
#| fig-link: https://www.nature.com/articles/s41467-021-24777-3#data-availability
knitr::include_graphics("files/code_availabilty.png")
```

And here is [\@coraloha](https://twitter.com/coraloha?s=20) reaction on Twitter. Some people get really jazzed about Open Science [{{< fa lightbulb >}}]{style="color: steelblue;"} ^[If you want to embed a tweet on a page, click on the ellipsis icon at the top of the tweet [{{< fa ellipsis-h size=xl >}}]{style="color: steelblue;"}, select [{{< fa code >}}]{style="color: steelblue;"}**Embed Tweet**, copy the HTML code provided, & plop it right into your `qmd` file. The only thing is you need to do is wrap the entire code in the HTML `<center>` & `</center>` tags.]
...
3. **Markdown PDF**: INSERT TEXT

4. **Markdown Presentation**: : INSERT TEXT
...
::: aside
Tiempo estimado de lectura: `r round(reading_time(), digits = 0)` minuto(s).
:::
...
> Si aún no lo has hecho, lee el breve post sobre [Cómo utilizar este sitio](../using-this-site/index.html) antes de continuar.

Utilizaremos [Quarto](https://quarto.org/)--un sistema de publicación científica y técnica de código abierto para construir nuestros productos web. Este sitio en sí no es sólo la guía para tus productos web, sino que también es ***creado con las mismas herramientas que utilizarás para tus proyectos***. Más información a continuación.x`x

En este módulo del curso aprenderemos a:

1. Configurar un entorno de trabajo instalando [R](https://www.r-project.org/), [RStudio](https://www.rstudio.com/), [Quarto](https://quarto.org/), y varios paquetes de R (p. ej,  [knitr](https://cran.r-project.org/web/packages/knitr/index.html), [tidyverse](https://www.tidyverse.org/), [reactable](https://glin.github.io/reactable/)).

2. Renderiza tablas de datos interactivas, figuras, gráficos, mapas, diagramas y mucho más.

3. Resuelve problemas utilizando búsquedas eficaces en Internet, [GitHub](https://github.com/) repositorios, foros como [Stack Overflow](https://stackoverflow.com/) y, por supuesto, nuestro ingenio.

4. Haz referencia a otras obras y crea artículos citables.

5. (si hay interés) Utiliza [git](https://git-scm.com/) y [Páginas de GitHub](https://pages.github.com/) para alojar nuestro trabajo (**esto no es un requisito del curso**).
...
Además, utilizaremos R Markdown y el framework Distill para crear lo que se denomina **estáticos** (en contraposición a **dinámico**). La diferencia es sencilla. *Dinámico* ^[A partir de ahora, utilizaré el término *dinámico* a lo largo de estas lecciones para referirme a un componente de un documento que hace algo o es interactivo, como una tabla que se desplaza o un mapa al que se puede hacer zoom. No confundas esto con un sitio web dinámico.] significa que cada vez que se carga una página, el contenido y el aspecto se adaptan al usuario, como en Facebook o Twitter. *Estático* significa que todos los que visitan el sitio ven lo mismo.

Sólo porque tu producto web sea *estático* no significa que sea aburrido. A diferencia de los documentos PDF, tus productos web tendrán componentes dinámicos e interactivos, como tablas, figuras, mapas, etc. Considera este ejemplo de Material Complementario que presentamos para un trabajo reciente. Este es el [versión pdf](files/Johnson_Supplementary_PDF.pdf) (escrita en R Markdown & [LaTeX](https://www.latex-project.org/)) y aquí está el [versión HTML](https://hypocolypse.github.io/som.html). Aunque el contenido de ambos documentos es idéntico, sus funcionalidades no lo son. Lo primero que hay que observar es que la mayoría de las tablas de datos del documento PDF (llamadas *Supplementary Data*) eran demasiado grandes para caber en el documento y, en su lugar, se suben como archivos de texto independientes al sitio web de la revista. En cambio, en la versión HTML, todas las tablas de datos están incrustadas en el documento.

Echa un vistazo al [Descripción del contenido](https://hypocolypse.github.io/som.html#content-description) y haz clic en cualquiera de los enlaces que dicen **Supplementary Data**. Los Datos suplementarios 2, por ejemplo, tienen 13 columnas y 911 filas, demasiado grandes para caber (cómodamente) en un documento PDF. Echa un vistazo a [Supplementary Data 3](https://hypocolypse.github.io/som.html#supplementary-data-3). Observa que puedes copiar o descargar el contenido de la tabla, ordenar, buscar (prueba a buscar por *invertebrados*) y ampliar la tabla completa (en **Show entries** selecciona *All*). Esta tabla contiene incluso hipervínculos que llevan a otra información importante. Adelante, haz clic en uno de los enlaces. Sigue adelante.
...
> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"}  Más del 70% de los investigadores ha intentado reproducir los experimentos de otro científico y no lo ha conseguido, y más de la mitad no ha logrado reproducir sus propios experimentos. Estas son algunas de las reveladoras cifras que se desprenden de la encuesta realizada por Nature a 1,576 investigadores que respondieron a un breve cuestionario en línea sobre la reproducibilidad en la investigación. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"} [@baker20161]

::: aside
A fecha de hoy, este artículo ha sido citado al menos `r cr_citation_count(doi = "10.1038/533452a")$count` veces [{{< fa lightbulb >}}]{style="color: steelblue;"} ^[Para obtener el número de citas de este artículo he utilizado la función `cr_citation_count` de la página  [`rcrossref`](https://cran.r-project.org/web/packages/rcrossref/index.html) paquete. La función toma un Identificador de Objeto Digital (DOI) como entrada y busca en CrossRef OpenURL para devolver el número de citas.]
:::
...
> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"}` … [is a] conjunto de procedimientos que permiten al lector de un artículo ver todo el proceso, desde los datos brutos y el código hasta las figuras y tablas. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"} [@goodman2016does]. 

O los EE.UU. National Science Foundation (NSF) lo define así ...

> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"}`  … se refiere a la capacidad de un investigador para duplicar los resultados de un estudio anterior utilizando los mismos materiales que utilizó el investigador original. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"} [@cacioppo2015social]

Por supuesto, todo esto es más fácil decirlo que hacerlo. Los investigadores interesados en llegar a un público más amplio y presentar ciencia transparente y reproducible se enfrentan a muchos retos; están sometidos a mucha presión para producir artículos y a menudo no se les da la oportunidad de dedicarse a estas actividades [@forrester2017next]. ¿Qué podemos hacer al respecto? Creo que un buen primer paso es crear productos web que adopten los conceptos de accesibilidad, transparencia y reproducibilidad. Los departamentos institucionales de divulgación y medios de comunicación hacen un trabajo increíble presentando los aspectos más destacados de la ciencia, pero no tienen tiempo ni recursos para profundizar mucho más. Creo que con un poco de organización, formación y apoyo todos podemos crear productos web que añadan valor a la ciencia,ya sea como herramientas educativas y de referencia, o como componentes de divulgación.

Pensemos de nuevo en el artículo que publicamos recientemente sobre un episodio de hipoxia aguda en Bocas del Toro, publicado en Nature Communications. Todas las revistas Nature exigen ahora a los autores que se adhieran a  [Reporting standards and availability of data, materials, code and protocols](https://www.nature.com/nature-portfolio/editorial-policies/reporting-standards).

> [{{< fa quote-left size=xl >}}]{style="color: steelblue;"} … los autores están obligados a poner a disposición de los lectores los materiales, datos, códigos y protocolos asociados sin reservas. [{{< fa quote-right size=xl >}}]{style="color: steelblue;"}
...
Y aquí está  [\@coraloha](https://twitter.com/coraloha?s=20) en Twitter. Algunas personas se entusiasman con la ciencia abierta  [{{< fa lightbulb >}}]{style="color: steelblue;"} ^[Si quieres incrustar un tuit en una página, haz clic en el icono de la elipsis situado en la parte superior del tweet [{{< fa ellipsis-h size=xl >}}]{style="color: steelblue;"}, select [{{< fa code >}}]{style="color: steelblue;"}**Embed Tweet**, copia el código HTML proporcionado y colócalo en tu archivo `qmd`. Lo único que tienes que hacer es envolver todo el código en el HTML `<center>` & `</center>` tags.]
...
[R](https://www.r-project.org/) es tanto un lenguaje de programación como un entorno de software para cálculo estadístico y gráficos. En algún momento, imagino que todos necesitaréis adoptar un entorno de programación para analizar vuestros datos y resumir vuestras conclusiones mediante figuras, tablas, etc. Ciertamente, R no es la única forma de hacerlo; sin embargo, creo que este entorno ofrece un valioso conjunto de herramientas para tus necesidades científicas. Las ventajas de R incluyen; **a**) es gratuito y de código abierto, **b**) sus capacidades se amplían mediante paquetes creados por los usuarios^[[The Comprehensive R Archive Network (CRAN)](https://cran.r-project.org/web/packages/), el repositorio central de software de R, contiene actualmente `r count_text` paquetes. Puedes encontrar cientos más en lugares como GitHub y Bioconductor], **c**) tiene una gran comunidad de usuarios (lo que significa que está bien soportado), **d**) es potente y flexible.

#### RStudio

[RStudio](https://www.rstudio.com/) es un entorno de desarrollo integrado ([IDE](https://en.wikipedia.org/wiki/Integrated_development_environment)) para el lenguaje R^[Las versiones recientes de RStudio tienen ahora la funcionalidad de ejecutar [código Python](https://www.rstudio.com/blog/three-ways-to-program-in-python-with-rstudio/) en tu documento R Markdown.]. Tómate un momento para familiarizarte con la idea de un IDE: en pocas palabras, RStudio proporciona un entorno de trabajo holístico para procesar código (R), generar figuras/tablas y crear sitios web.

> Imagina un coche. Piensa en R como el motor y en el IDE RStudio como el salpicadero.

#### R Markdown

[R Markdown](https://rmarkdown.rstudio.com/) es realmente el pan de cada día de lo que vamos a hacer. R Markdown es un formato de archivo (`.Rmd`) para crear documentos dinámicos con R. R Markdown combina la sintaxis de Markdown con el lenguaje (y el entorno) de R. Los documentos R Markdown se escriben en [Markdown](https://en.wikipedia.org/wiki/Markdown)\---a *ligero* [lenguaje de marcado](https://en.wikipedia.org/wiki/Markup_language) (como HTML), que utiliza una sintaxis relativamente sencilla para facilitar el *transformación* de archivos de texto legibles por humanos en `.html` o `.pdf` documentos. Lo que esto significa es que en lugar de escribir código HTML y CSS para hacer un sitio web, escribes tu contenido en Markdown, que luego es traducido (por RStudio en este caso) a contenido web. El código (R) de tu documento se incrusta dentro de [code chunks](https://rmarkdown.rstudio.com/lesson-3.html). Durante la construcción de una página o sitio, RStudio identifica los trozos de código, ejecuta el código, traduce los resultados a Markdown y, a continuación, renderiza el resultado en un archivo HTML.
...
A lo largo de este curso de campo, todos diseñaréis y realizaréis experimentos, haréis observaciones y mediciones, recopilaréis datos, etc., a menudo formando parte de un grupo con otros compañeros. Tu trabajo en este módulo del curso consiste en tomar todas las partes de tus proyectos -hipótesis, información de fondo, métodos, resultados y conclusiones- y plasmar estos elementos en un informe. [R Markdown](https://rmarkdown.rstudio.com/) utilizando la herramienta [Destilar](https://rstudio.github.io/distill/) de Distill.

> [{{< fa edit >}}]{style="color: steelblue;"} Regardless of whether you are part of a group or running solo, you will each create a web product that contains the details for **at least two projects** you work on in the course.
...
Por último, estaré encantada de escribir tutoriales y lecciones adicionales que cubran material no incluido ya en el sitio.
Así que, por favor, hazme saber si hay algo más que quieras aprender y escribiré una entrada para que otros estudiantes puedan seguirme.
...
Me gustaría que hicieras una cosa más antes de terminar. Vuelve a recorrer esta página desde la parte superior. No hace falta que vuelvas a leer el contenido (bostezo), sólo quiero que tomes nota de los distintos elementos: el formato del texto (por ejemplo, *cursiva* y **negrita**), encabezados, [índice](https://rstudio.github.io/distill/basics.html#table-of-contents), [notas a pie de página y comentarios](https://rstudio.github.io/distill/basics.html#appendices) hipervínculos, [apéndices](https://rstudio.github.io/distill/basics.html#appendices), [citas](https://rstudio.github.io/distill/citations.html) imágenes, etc. Incorporarás muchos de estos elementos en tus productos web. No te preocupes por los detalles ahora, los trataremos con más detalle más adelante.
